...
The intended deployment is a one-shot container on `otter`, alongside (but separate from) the Qdrant and Ollama stacks. Tagged releases publish an image to GHCR (`ghcr.io/onemorerabbit/ingstr-app`) via [.github/workflows/release.yml](.github/workflows/release.yml). On otter, pull the image and invoke ingest with `docker compose run`:
...
```bash
git clone https://github.com/OneMoreRabbit/ingstr-app.git
cd ingstr
python -m venv .venv
